...
// Type
type FilmInfoType = {
    filmInfo: MovieDetailType;
    castList: Array<CastType>;
};

// Component
export const FilmInfo: FC<FilmInfoType> = ({ filmInfo, castList }) => {
    // Templates
    return (
        <div
            ...
            style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original/${filmInfo?.backdrop})` }}
        >
            <div className="flex items-start -mx-4 max-h-fit">
                <div className="hidden md:block w-64 lg:w-96 px-4">
                    <img src={`https://image.tmdb.org/t/p/w500/${filmInfo?.poster}`} className="w-full rounded-3xl" />
                </div>
